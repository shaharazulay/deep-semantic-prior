...
from net import *
from net.losses import StdLoss, MultiScaleLoss, YIQGNGCLoss, GradientLoss, ExtendedL1Loss, GrayLoss
...
class Segmentation(object):
    def __init__(self, image_name, image, plot_during_training=True,
                 zero_step_iter_num=2000,
                 first_step_iter_num=2000,
                 second_step_iter_num=4000,
                 bg_hint=None, fg_hint=None,
                 show_every=500,
                 downsampling_factor=0.1, downsampling_number=0, output_path="output/"):
        self.image = image
    ...
    def _init_nets(self):
        ...
        self.rec_net = rec_net.type(torch.cuda.FloatTensor)
    ...
    def optimize(self):
        ...
        if self.fg_hint is None and self.bg_hint is None:
            # step 0
            optimizer = torch.optim.Adam(self.parameters, lr=self.learning_rate)
            for j in range(self.zero_step_iter_num):
                optimizer.zero_grad()
                self._step0_optimization_closure(j)
    ...
    def finalize_first_step(self):
        ...
        save_image(self.image_name + "_1_left", left, self.output_path)
        save_image(self.image_name + "_1_right", right, self.output_path)
        save_image(self.image_name + "_hint1", self.bg_hint, self.output_path)
        save_image(self.image_name + "_hint2", self.fg_hint, self.output_path)
        save_image(self.image_name + "_hint1_masked", self.bg_hint * self.image, self.output_path)
        save_image(self.image_name + "_hint2_masked", self.fg_hint * self.image, self.output_path)

    def finalize_zero_step(self):
        rec_image = torch_to_np(self.rec_net(self.images_torch[0]))
        save_image(self.image_name + "_rec", rec_image, self.output_path)
        save_image(self.image_name + "_hint1", self.bg_hint, self.output_path)
        save_image(self.image_name + "_hint2", self.fg_hint, self.output_path)
        save_image(self.image_name + "_hint1_masked", self.bg_hint * self.image, self.output_path)
        save_image(self.image_name + "_hint2_masked", self.fg_hint * self.image, self.output_path)
    ...
    def finalize(self):
        save_image(self.image_name + "_left", self.best_result.left, self.output_path)
        save_image(self.image_name + "_learned_mask", self.best_result.learned_mask, self.output_path)
        save_image(self.image_name + "_right", self.best_result.right, self.output_path)
        save_image(self.image_name + "_original", self.images[0], self.output_path)
        # save_image(self.image_name + "_fg_bg", ((self.fg_hint - self.bg_hint) + 1) / 2)
        save_image(self.image_name + "_mask", self.best_result.mask, self.output_path)

    def _update_hints(self):
        rec_image = torch_to_np(self.rec_net(self.images_torch[0]).squeeze(0))
        saliency = StaticSaliencySpectralResidual_create()
        _, saliency_map_rec = saliency.computeSaliency(rec_image)
        saliency_map_rec = (saliency_map_rec * 255).astype("uint8")
        saliency_map_rec = cv2.threshold(saliency_map_rec, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]

        ### DEBUG ###
        _, saliency_map_orig = saliency.computeSaliency(torch_to_np(self.images_torch[0].squeeze(0)))
        saliency_map_orig = (saliency_map_orig * 255).astype("uint8")
        saliency_map_orig = cv2.threshold(saliency_map_orig, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        save_image(self.image_name + "_orig_saliency", np.expand_dims(saliency_map_orig, 0), self.output_path)
        ### DEBUG ###
        # # TODO: change to choose randomly?
        # bg_value = rec_image[0, :, 0, 0].view(1, -1, 1, 1).repeat(1, 1, *rec_image.size()[2:])
        # dist_bg = torch.sum((rec_image - bg_value) ** 2, dim=1)
        # fg_index = torch.argmax(dist_bg)
        # fg_index = np.unravel_index(fg_index.cpu().numpy(), dist_bg.size())
        # fg_value = rec_image[0, :, fg_index[1], fg_index[2]].view(1, -1, 1, 1).repeat(1, 1, *rec_image.size()[2:])
        # dist_fg = torch.sum((rec_image - fg_value) ** 2, dim=1)
        # fg_mask = torch.zeros(1, 1, *rec_image.size()[2:])
        # fg_mask[0, dist_fg < dist_bg] = 1.
        # bg_mask = 1 - fg_mask
        # self.fg_hint = torch_to_np(fg_mask)
        # self.bg_hint = torch_to_np(bg_mask)
        self.fg_hint = np.expand_dims(saliency_map_rec / 255., 0)
        self.bg_hint = 1. - self.fg_hint
        # to init hints
        self._init_images()
    ...
    def _initialize_step0(self, iteration):
        self._initialize_any_step(iteration)
        self.rec_image = self.rec_net(self.images_torch[0])
    ...
    def _step0_optimization_closure(self, iteration):
        ...
        :return:
        """
        self._initialize_step0(iteration)
        #for rec_image, original_image_torch in zip(self.rec_image, self.images_torch):
        self.total_loss += self.l1_loss(self.rec_image, self.images_torch[0])
            #self.total_loss += self.mse_loss(rec_image, original_image_torch)
    ...
    def _finalize_iteration(self):
        # TODO: you miss the last iteration
        ...
        if self.fg_hint is not None and self.bg_hint is not None:
            plot_image_grid("left_right_hints_{}".format(name),
                            [np.clip(self.fg_hint, 0, 1),
                             np.clip(self.bg_hint, 0, 1)], self.output_path)
        for i, (left_out, right_out, mask_out, image) in enumerate(zip(self.left_net_outputs,
            ...
            plot_image_grid("left_right_{}_{}".format(name, i),
                            [np.clip(torch_to_np(left_out), 0, 1),
                             np.clip(torch_to_np(right_out), 0, 1)], self.output_path)
            mask_out_np = torch_to_np(mask_out)
            plot_image_grid("learned_mask_{}_{}".format(name, i),
                            [np.clip(mask_out_np, 0, 1), 1 - np.clip(mask_out_np, 0, 1)], self.output_path)
