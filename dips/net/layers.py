...
class BlurLayer(nn.Module):
    def __init__(self, shape):
        """
        We assume that the shape is odd in both directions
        :param shape: tuple (x, y)
        """
        super(BlurLayer, self).__init__()
        self.shape = shape
        to_pad_x = int((self.shape[0] - 1) / 2)
        to_pad_y = int((self.shape[1] - 1) / 2)
        self.pad = nn.ReflectionPad2d((to_pad_x, to_pad_x, to_pad_y, to_pad_y))
        mask = np.zeros(self.shape)  # np.ones(self.shape) / (self.shape[0] * self.shape[1])
        self.mask_np = mask.reshape(1, 1, mask.shape[0], mask.shape[1])
        self.mask = nn.Parameter(data=torch.cuda.FloatTensor(self.mask_np), requires_grad=True)
