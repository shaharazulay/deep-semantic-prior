...
from utils.colors import rgb_to_yiq
...
class MultiScaleLoss(nn.Module):
    def __init__(self, downsample_factors):
        ...
        self.ssim = SSIM().cuda()
        self.downsampling_factors = downsample_factors

    def forward(self, x):
        """
        x is a list of images, we verify that each one is consistent with the first one
        """
        ssim = 0
        j = 0
        for i in self.downsampling_factors:
            j += 1
            downsampler = Downsampler(n_planes=x[0].shape[1], factor=i,
                                      kernel_type='lanczos2', phase=0.5, preserve_size=True).cuda()
            ssim += self.ssim(crop_torch_image(downsampler(x[0])), x[j])
        return ssim / j


class GNGCLoss(nn.Module):
    def __init__(self, shape=5):
        super(GNGCLoss, self).__init__()
        self.shape = shape
    ...
    def forward(self, x, y):
        if x.shape[1] == 3:
            x_g = rgb_to_yiq(x)  # take the Y part
            y_g = rgb_to_yiq(y)  # take the Y part
            c1 = torch.mean(self.covar(x_g[:, :1, :, :], y_g[:, :1, :, :]) ** 2)
            vv1 = torch.mean(self.var(x_g[:, :1, :, :]) * self.var(y_g[:, :1, :, :]))
            c2 = torch.mean(self.covar(x_g[:, 1:2, :, :], y_g[:, 1:2, :, :]) ** 2)
            vv2 = torch.mean(self.var(x_g[:, 1:2, :, :]) * self.var(y_g[:, 1:2, :, :]))
            c3 = torch.mean(self.covar(x_g[:, 2:, :, :], y_g[:, 2:, :, :]) ** 2)
            vv3 = torch.mean(self.var(x_g[:, 2:, :, :]) * self.var(y_g[:, 2:, :, :]))
            return c1 / vv1 + c2 / vv2 + c3 / vv3
        else:
            assert x.shape[1] == 1
